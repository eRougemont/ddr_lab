...
Query query = new TermQuery(new Term(ALIX_TYPE, CHAPTER));
...
for (ScoreDoc src : biblio) {
    ...
    for (ScoreDoc tgt : mltDocs) {
        ...
        if (first) {
            out.println("  <dt>" + " <a target=\"left\" href=\"text.jsp?id=" + srcDoc.get(ALIX_ID) + "\"> "
            + ML.detag(srcDoc.get("bibl")).trim() + "</a>" + "</dt>");
            first = false;
        }
        Document tgtDoc = reader.document(tgt.doc, DOC_SHORT);
        out.println("    <dd>" + tgt.score + " <a target=\"right\" href=\"text.jsp?id=" + tgtDoc.get(ALIX_ID) + "\">"
